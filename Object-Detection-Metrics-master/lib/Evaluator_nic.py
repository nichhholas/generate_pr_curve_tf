...
class Evaluator:
    def GetPascalVOCMetrics(self,
        ...
        """
        print("Getting PascalVOCMetrics")
        ...
        for c in classes:
            #### Get filenames ####
            filenames_gt = []
            [filenames_gt.append(g[0]) for g in groundTruths if g[1] == c]

            # filenames_det = []
            # [filenames_det.append(d[0]) for d in detections if d[1] == c]

            # confidence = []
            # [confidence.append(d[2]) for d in detections if d[1] == c]
            ...
            filenames_det = []
            [filenames_det.append(d[0]) for d in dects if d[1] == c]

            confidence = []
            [confidence.append(d[2]) for d in dects if d[1] == c]
            ...
            for d in dects:
                ...
                xmax.append(bbox[2])
                ymax.append(bbox[3])
                asp_ratio.append( (bbox[2]-bbox[0])/ (bbox[3]-bbox[1]))
                rel_size.append( (bbox[2]-bbox[0])*(bbox[3]-bbox[1]) )

            for g in gts:
                gt_bbox = g[3]
                print(gt_bbox)
                gt_xmin.append(gt_bbox[0])
                gt_ymin.append(gt_bbox[1])
                gt_xmax.append(gt_bbox[2])
                gt_ymax.append(gt_bbox[3])
                gt_asp_ratio.append( (gt_bbox[2]-gt_bbox[0])/ (gt_bbox[3]-gt_bbox[1]))
                gt_rel_size.append( (gt_bbox[2]-gt_bbox[0])*(gt_bbox[3]-gt_bbox[1]) )


            # [bbox_coords.append(d[3]) for d in dects if d[1]==c]
            ...
            counter = 0
            gt_found = []
            print('Total number of gts ', len(gts))
            for d in range(len(dects)):
                ...
                print("Number of gt ", len(gt))

                if iouMax >= IOUThreshold:
                    
                    if det[dects[d][0]][jmax] == 0:
                        gt_found.append(idx[jmax])
            ...
            print("Number of GTs not picked up on", (len(gts) - len(gt_found)))
            acc_FP = np.cumsum(FP)
            acc_TP = np.cumsum(TP)
            rec = acc_TP / npos
            prec = np.divide(acc_TP, (acc_FP + acc_TP))

            ### Write table to json to be imported ###
            import json
            ...
            json_dic = {
                            'image_gt': filenames_gt, 
                            'image_det': filenames_det, 
                            'TP': TP,
                            'FP': FP, 
                            'npos': npos, 
                            ...
                            'gt_xmax': gt_xmax,
                            'gt_ymax': gt_ymax,
                            'gt_asp_ratio': gt_asp_ratio,
                            'gt_rel_size': gt_rel_size,
                            'gt_found': gt_found

                        }

            json_string = json.dumps(json_dic, cls= MyEncoder)
            print("Writing json file v2")
            with open(c+'.json','w') as f:
                f.write(json_string)
            print("Wrote to "+c+'.json')
